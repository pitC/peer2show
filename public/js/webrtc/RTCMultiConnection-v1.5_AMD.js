        ...
        function newPrivateSocket(_config) {
            ...
            var peerConfig = {
                ...
                onmessage: function(data) {
                	console.log("peer config onmessage");
                	console.log(data);
                    config.onmessage({
                        data: data,
            ...
            reader.onloadend = function(evt) {
                if (evt.target.readyState == FileReader.DONE) {
                    addChunks(file.name, evt.target.result, function() {
                        sliceId++;
                        console.log("addChunksCB:"+sliceId+" "+sliceSize+" "+file.size);
                        if ((sliceId + 1) * sliceSize < file.size) {
                            ...
                            console.log("addChunksCB: socket.send done!");
            ...
            function addChunks(fileName, binarySlice, callback) {
                ...
                for (var i = 0; i < numOfChunksInSlice; i++) {
                    ...
                    FileConverter.ArrayBufferToDataURL(chunks[currentPosition], function(str) {
                        ...
                        console.log("RTC 1.5 sent chunk "+currentPosition+"/"+maxChunks);
                    });
...
//                    setTimeout(callback, config.interval);
    ...
    var FileSender = {
        send: function(config) {
            ...
            var interval = 100;
            if (!root.preferSCTP && !moz) interval = 100;
